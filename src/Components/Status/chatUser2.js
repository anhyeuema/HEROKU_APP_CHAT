...
export default class ChatUser2 extends Component {
    constructor(props) {
        super(props);
    ...
    globalGetUser(Useranme1) {
        ...
        this.socket.on('server-send-socket.id+Username', async (ArraySocketUsername) => {
            ...
            this.socket.on('server-send-messenger', async (dataMessenger) => {
                ...
                //   e.setState({ soOnMess: ((this.state.soOnMess) + 1) })
                ...
                await this.socket.on('server-trave-yeucau-ArrayMess-User', async (DataMessengerApp_r) => {
                    //{Nms: n, Sms: SaveDataMessengerApp}
                   // console.log('server-trave-yeucau-ArrayMess-User:socket.on tin nhan:', DataMessengerApp_r);
                    ...
                    await e.setState({ soOnMess: Nms });
                    const { soOnMess } = this.state;
                  //  console.log('soOnMess::::', soOnMess);
                    ...
                    await dataMessenger.map( async(dataEmit, index) => {
                        var UserSendKey = await {
                      ...
                      //  console.log('.UserSendKey: ==0:::', UserSendKey);
                        await ArrMessSendServer.push(UserSendKey);
                ...
                var ArrayMessUsersendUserItem = await {
               ...
               // await this.socket.emit('client-send-ArrayMessUsersendUserItem', ArrayMessUsersendUserItem);
    
    
               // console.log('ArrayMessUsersendUserItem truong hop == 011111111111111 tra ve', ArrayMessUsersendUserItem);
                */
              
               await this.socket.emit('client-send-ArrayMessUsersendUserItem', dataMessenger);
               console.log('client-send-ArrayMessUsersendUserItem', dataMessenger);
    ...
    hienthiMess() {
        ...
        this.socket.on('server-send-messenger', async (dataMessenger) => {
            console.log('dataMessenger hienthiMess:::', dataMessenger);
            await e.setState({
                UserWeb: dataMessenger[0].UsernameNguoiSend,
                UserApp: dataMessenger[0].UsernameNguoiNhan,
                soPage: 1, //de hien thi la ra tin nhan moc ve 1 tinh tu length max - 1.m = hien thi so phan tu tu lon nhat toi -m phan tu
        ...
        return (
            <View style={{ flex: 1, backgroundColor: '#fff' }}>
                ...
                <View style={{ flex: 1, backgroundColor: '#fff', flexDirection: 'row' }} >
                    <Text style={styles.styleText} >User2</Text>
                    <Text style={styles.UserOnline}>User22: {this.state.Username === null ? null : this.state.Username}</Text>
